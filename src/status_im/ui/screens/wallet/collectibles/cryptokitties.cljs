...
(defn update-kitty [id m v]
  (map #(if (= (:id %) id) (assoc % :bio (:bio m)) %) v))

(handlers/register-handler-fx
 :load-kitty-success
 [re-frame/trim-v]
 (fn [{db :db} [[id collectible]]]
   {:db (update db :collectibles #(update-kitty id collectible %))}))
...
(defn kitties-url [address]
  (str "https://api.cryptokitties.co/kitties?offset=0&limit=12&owner_wallet_address=" address "&parents=false"))

(handlers/register-handler-fx
 :load-kitties-success
 (fn [{db :db} [_ collectibles]]
   {:db db
    :dispatch-n [[:load-kitties (map :id collectibles)]
                 [:load-collectibles-success {:collectibles collectibles}]]}))

(defmethod collectibles/load-collectibles-fx :CK [_ address]
  {:http-get {:url                   (kitties-url address)
              :success-event-creator (fn [o]
                                       [:load-kitties-success (:kitties (parse-payload o))])
              :failure-event-creator (fn [o]
...
(defmethod collectibles/render-collectible :CK [_ {:keys [id bio image_url] :as m}]
  [react/view {:style view-style}
   ...
   [action-button/action-button {:label               (i18n/label :t/view-cryptokitties)
                                 ...
                                 :on-press            #(.openURL react/linking (str "https://www.cryptokitties.co/kitty/" id))}]])
